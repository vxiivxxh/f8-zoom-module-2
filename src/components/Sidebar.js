  ...
  return `
    <aside class="w-sidebar h-full bg-yt-sidebar text-yt-text-secondary flex flex-col z-sidebar border-r border-gray-800 transition-all duration-300">
      ...
      <div class="flex items-center gap-4 px-4 pl-6 mb-2 h-16 shrink-0">
        ...
        <a href="/" class="flex items-center justify-center p-2 logo-container shrink-0" data-navigo>
      ...
      <nav class="flex-1 px-3 space-y-1 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-600">
        ...
        <!-- Điều hướng chính -->
        <a href="/" class="flex items-center gap-5 px-4 py-3 rounded-lg hover:bg-yt-hover hover:text-white transition-colors data-navigo" data-navigo>
          <svg class="w-6 h-6" fill="currentColor" viewBox="0 0 24 24"><path d="M10 20v-6h4v6h5v-8h3L12 3 2 12h3v8z"/></svg>
          <span class="font-medium text-sm">Trang chủ</span>
        </a>
        <a href="/explore" class="flex items-center gap-5 px-4 py-3 rounded-lg hover:bg-yt-hover hover:text-white transition-colors data-navigo" data-navigo>
        ...
        <a href="/library" class="flex items-center gap-5 px-4 py-3 rounded-lg hover:bg-yt-hover hover:text-white transition-colors data-navigo" data-navigo>
        ...
        <div class="space-y-1 hide-on-collapse">
             <a href="#" class="flex items-center gap-4 px-4 py-2 rounded-lg hover:bg-yt-hover hover:text-white transition-colors group">
                 <!-- Đảm bảo biểu tượng căn chỉnh đẹp mắt -->
...
export const setupSidebarEvents = (router) => {
  ...
  // Logic nút bật/tắt (Placeholder hiện có hoặc mới)
  const toggleBtn = document.getElementById("sidebar-toggle");
  if (toggleBtn) {
    toggleBtn.addEventListener("click", () => {
      const sidebar = document.querySelector("aside");
      if (sidebar) {
        // Logic bật/tắt đơn giản nếu cần, hoặc dựa vào các cách tiếp cận dựa trên CSS hiện có
        // Thông thường bật/tắt sidebar có thể ngụ ý thu gọn.
        // Hiện tại, tập trung vào Logo như yêu cầu.
      }
    });
  }

  // Cập nhật tất cả các liên kết điều hướng để hoạt động với đường dẫn cơ sở của Navigo
